...
const usersss = require('../Model/Owner' );
...
router.post('/reg', async (req, res) => {
    try {
        ...
        const roles= "Owner";
        await usermode.create({ name, password: hashpwd, email,doctorid,role:roles });
        ...
        const mailOptions = {
            ...
            html: `
              <div style=  color: black ; font-size: 20px;">
                ...
                <p>Don't Share OTP at any cost</p>
...
router.post("/res", async (req, res) => {
    ...
    try {
        ...
        const user = await usersss.findOne({ email });
        if (!user) {
...
router.post("/respwd", async (req, res) => {
    ...
    try {
        ...
        const user = await usersss.findOne({
            resetPwdToken: token,
